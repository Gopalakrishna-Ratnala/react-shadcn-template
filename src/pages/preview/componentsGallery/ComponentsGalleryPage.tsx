...
import { ThemeHistoryPanel, themeLogSchema } from "./components";
...
/**
 * cmdk's <Command> always calls `scrollIntoView()` on its pre-selected first
 * item the moment it mounts (so a command palette opened via a dialog starts
 * scrolled to the top of its own list). Rendered inline on this long page —
 * not inside a dialog/popover — that scroll bubbles all the way up to the
 * window, dragging the whole page down to wherever this demo happens to sit.
 * Suppress `scrollIntoView` calls originating from inside this subtree for
 * one frame (long enough for that mount-time call to fire and be swallowed),
 * then restore the real implementation for everything else on the page.
 */
const SuppressScrollJumpOnMount = ({
  children,
}: {
  children: ReactNode;
}): ReactElement => {
  ...
  useEffect(() => {
    ...
    Element.prototype.scrollIntoView = function (
      ...
      if (ref.current?.contains(this)) return;
...
const NAV_LINKS: NavLink[] = [
  ...
  { id: "theme-history", label: "14 · Theme history" },
];
          ...
          <article className={styles.mainStyles}>
            ...
            <section id="form-inputs" className={styles.sectionStyles}>
              ...
              <section className={styles.specimenGridStyles}>
                ...
                <Specimen
                  title="Combobox"
                  description="Search with live results"
                >
                  <Combobox items={FRAMEWORKS} defaultValue="Vite + React">
                    <ComboboxInput
                      className={styles.constrainedStyles}
            ...
            <section id="overlays" className={styles.sectionStyles}>
              ...
              <section className={styles.specimenGridStyles}>
                <Specimen title="Popover" description="Opens on click">
                  <Popover modal={false}>
                    <PopoverTrigger
                      render={
                ...
                <Specimen title="Tooltip" description="Visible on hover/focus">
                  <Tooltip>
                    <TooltipTrigger
                      render={
                        <Button
            ...
            <section id="menus" className={styles.sectionStyles}>
              ...
              <section className={styles.specimenGridStyles}>
                <Specimen
                  title="Dropdown menu"
                  description="Opens with a submenu"
                >
                ...
                <Specimen title="Menubar" description="Click a menu to open it">
                  <Menubar>
                    <MenubarMenu modal={false}>
            ...
            {/* ============ 14. THEME HISTORY ============ */}
            <section id="theme-history" className={styles.sectionStyles}>
              <header className={styles.sectionHeaderStyles}>
                <small className={styles.sectionKickerStyles}>Section 14</small>
                <h2 className={styles.sectionTitleStyles}>Theme history</h2>
                <p className={styles.sectionDescStyles}>
                  Every theme candidate ever created, read directly from
                  THEME-LOG.json — see styling/shadcn/03-theme-versioning.md.
                </p>
              </header>
